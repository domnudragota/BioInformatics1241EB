...
fn main() -> Result<()> {
    println!("[lab03] ex02_tm_window (single-pass, k=9, FASTA .fa/.fasta/.gz)");

    // CLI:
    // ex02_tm_window <input.fa[.gz]>
    //   [--method wallace|salt] [--na 0.05]
    //   [--out tm.png] [--width 1200] [--binw 10000] [--progress]
    ...
    let mut out_path: String = "tm_plot.png".to_string();
    let mut width_target: usize = 1200;   // final plotted width (bins)
    ...
    while let Some(flag) = args.next() {
        match flag.as_str() {
            ...
            "--out" => {
                out_path = args.next().unwrap_or_else(|| "tm_plot.png".to_string());
            }
            "--width" => {
                width_target = args
                    .next()
    ...
    // If we produced more bins than desired width, merge neighbors down to `width_target`
    let (bins_final, y_min, y_max) = downsample_bins(&stats.bins, width_target);

    // Build X,Y for plotting
    ...
    plot_line(
        ...
        (y_min, y_max),
        &out_path,
        "Tm (k=9) sliding window — downsampled",
        "Position (bin)",
        "Tm (°C)",
    )?;
...
/// Minimal bin to aggregate values for plotting
#[derive(Clone, Copy)]
struct BinAgg {
    sum: f64,
    min: f64,
...
/// Aggregated stats across all bins for single-pass
struct StreamStats {
    bins: Vec<BinAgg>,
    total_valid: usize,
    n_windows: usize,
...
/// Single-pass streamed computation with online binning
fn compute_single_pass<P: AsRef<Path>>(
    ...
    // Precompute constants (avoid per-window divisions/log10)
    ...
    // Ring of "is GC" flags (0/1) for last k valid bases (k=9 -> fixed array is fastest)
    ...
    // FASTA header skipping state machine (works with chunked reads)
            ...
            if let Some(f) = gc_flag(b) {
                ...
                if ring_filled < k {
                    ...
                    if ring_filled == k {
                        ...
                        current.push(tm);
                        if tm < global_min { global_min = tm; }
                        if tm > global_max { global_max = tm; }
                        n_windows += 1;
                    }
                } else {
...
/// Merge/average bins down to at most `target` bins; also return min/max over the *final* bins.
fn downsample_bins(bins: &[BinAgg], target: usize) -> (Vec<BinAgg>, f64, f64) {
    if bins.is_empty() {
        return (vec![], 0.0, 1.0);
    }
    if bins.len() <= target {
        // Recompute min/max based on averages for accurate plot range
    ...
    // Merge groups of ~ceil(len/target)
    let group = (bins.len() + target - 1) / target;
...
/// Compute Tm from GC count using precomputed constants.
#[inline]
...
/// Open a buffered reader for path (gz or plain).
...
/// Simple line plot (avg per bin).
fn plot_line(xs: &[i32], ys: &[f64], y_range: (f64, f64), out: &str, title: &str, xlab: &str, ylab: &str) -> Result<()> {
    let w: u32 = 1200;
    let h: u32 = 420;
    ...
    let series = xs.iter().zip(ys.iter()).map(|(x, y)| (*x, *y));
